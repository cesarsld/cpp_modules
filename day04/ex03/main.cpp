/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   main.cpp                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: cjaimes <cjaimes@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/01 02:52:31 by cjaimes           #+#    #+#             */
/*   Updated: 2021/05/01 03:42:35 by cjaimes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "AMateria.hpp"
#include "IMateriaSource.hpp"
#include "MateriaSource.hpp"
#include "ICharacter.hpp"
#include "Character.hpp"
#include "Ice.hpp"
#include "Cure.hpp"
#include <string>
#include <iostream>


int main() {

	std::cout << "1. Creating MateriaSource and learning two materias" << std::endl;
		IMateriaSource* src = new MateriaSource();
		src->learnMateria(new Ice());
		src->learnMateria(new Cure());
	std::cout << std::endl;

	std::cout << "2. Creating Character \"mario\" and equipping materials" << std::endl;
		Character* mario = new Character("mario");
		AMateria* tmp;
		tmp = src->createMateria("ice");
		mario->equip(tmp);
		tmp = src->createMateria("cure");
		mario->equip(tmp);
	std::cout << std::endl;

	std::cout << "3. Creating ICharacter \"mario2\" from a copy of mario and deleting mario to prove deep copy" << std::endl;
		ICharacter* mario2 = new Character(*mario);
		delete mario;
	std::cout << std::endl;

	std::cout << "4. creating ice and cure and equipping more (testing with too much) Materia" << std::endl;
		AMateria *cure = new Cure;
		AMateria *ice = new Ice;

		mario2->equip(cure);
		mario2->equip(ice);
		mario2->equip(cure);
		mario2->equip(ice);
		std::cout << "Ice spell (equipped on 3) XP is: " << ice->getXP() << std::endl;
		std::cout << "Cure spell (equipped on 2) XP is: " << cure->getXP() << std::endl;
	std::cout << std::endl;

	std::cout << "5. using all equiped materias on new ICharacter bob" << std::endl;
		ICharacter *bob = new Character("Bob");
		mario2->use(0, *bob);
		mario2->use(1, *bob);
		mario2->use(2, *bob);
		mario2->use(3, *bob);
		mario2->use(10, *bob);
		mario2->use(-1, *bob);
		std::cout << "Ice spell (equipped on 3) XP is: " << ice->getXP() << std::endl;
		std::cout << "Cure spell (equipped on 2) XP is: " << cure->getXP() << std::endl;
	std::cout << std::endl;


	std::cout << "6. unequiping materias 2 and 3 and trying again" << std::endl;
		mario2->unequip(2);
		mario2->unequip(3);

		mario2->use(0, *bob);
		mario2->use(1, *bob);
		mario2->use(2, *bob);
		mario2->use(3, *bob);
		mario2->use(10, *bob);
		mario2->use(-1, *bob);
		std::cout << "Ice spell (unequipped) XP is: " << ice->getXP() << std::endl;
		std::cout << "Cure spell (unequipped) XP is: " << cure->getXP() << std::endl;
	std::cout << std::endl;

	std::cout << "7. requipping materias 2 and 3 in new order and trying again" << std::endl;
		mario2->equip(ice);
		mario2->equip(cure);

		mario2->use(0, *bob);
		mario2->use(1, *bob);
		mario2->use(2, *bob);
		mario2->use(3, *bob);
		mario2->use(10, *bob);
		mario2->use(-1, *bob);
		std::cout << "Ice spell (equiped on 2) XP is: " << ice->getXP() << std::endl;
		std::cout << "Cure spell (equiped on 3) XP is: " << cure->getXP() << std::endl;
	std::cout << std::endl;

	std::cout << "8. Deleting bob, mario2 and the MateriaSource" << std::endl;
		delete bob;
		delete mario2;
		delete src;
	std::cout << std::endl;

	return (0);
}
